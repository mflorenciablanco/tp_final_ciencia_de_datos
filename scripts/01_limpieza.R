...
#ver distribucion de NA
# contando NAs por variable
datos_filtrados %>%
...
#crecimiento del PIB (pbi_crec) no están disponibles, pero el año y el país estan registrados
#esto sugiere un problema de cobertura de indicadores para ciertos países o años

dev.off()

#análisis de distribucion NA
...
#el p value> 0.05, no rechazamos H0.por lo tanto, no podemos afirmar que son MAR
#la variable observada (pbi_crec) NO es un buen predictor de si la variable 
#clave (apertura_comercial) faltará

#test con IED 

t.test(ied_pct_pbi ~ na_apertura, data = datos_filtrados)

#ahora p value< 0.05 y rechazamos H0. Los datos faltantes de apertura comercial
#son MAR con respecto a la IED


#utilizamos imputacion multiple como estrategia para reemplazar los NA
datos_filtrados <- datos_filtrados %>%
  select(-na_apertura)

#seleccionamos solo las variables relevantes para el modelo MICE
#MICE automáticamente usa las completas (country, year) para predecir las faltantes

#ejecutamos la imputación múltiple
