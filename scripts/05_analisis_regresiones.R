...
#analisis de correlacion: crecimiento del PBI (nivel) vs Apertura Comercial (log)

#grafico de dispersion (Scatter Plot)

#visualizamos la relacion Log-Linear (Y en nivel, X en Log)
...
modelo <- lm(pbi_crec ~ apertura_comercial_log, data = df_analisis)
...
bp_white <- bptest(modelo, ~ apertura_comercial_log + I(apertura_comercial_log^2), data = df_analisis)
print(bp_white)

#conclusion: los datos no presentan heterocedasticidad
